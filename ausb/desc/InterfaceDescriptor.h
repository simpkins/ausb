...
public:
  ...
  constexpr uint8_t interface_number() const { return data_[2]; }

  constexpr InterfaceDescriptor &set_alt_setting(uint8_t value) {
    data_[3] = value;
    return *this;
  }
  constexpr uint8_t alt_setting() const { return data_[3]; }

  constexpr InterfaceDescriptor &set_num_endpoints(uint8_t num) {
    data_[4] = num;
    return *this;
  }
  constexpr uint8_t num_endpoints() const { return data_[4]; }

  constexpr InterfaceDescriptor &set_class(UsbClass usb_class, uint8_t subclass,
                                        uint8_t protocol) {
  ...
  constexpr InterfaceDescriptor &set_class(uint8_t usb_class, uint8_t subclass,
                                        uint8_t protocol) {
    set_class(usb_class);
    set_subclass(subclass);
    set_protocol(protocol);
  ...
  constexpr uint8_t get_class() const { return data_[5]; }

  constexpr InterfaceDescriptor &set_subclass(uint8_t subclass) {
    data_[6] = subclass;
    return *this;
  }
  constexpr uint8_t subclass() const { return data_[6]; }

  constexpr InterfaceDescriptor &set_protocol(uint8_t protocol) {
    data_[7] = protocol;
    return *this;
  }
  constexpr uint8_t protocol() const { return data_[7]; }
  ...
  constexpr uint8_t string_index() const { return data_[8]; }
...
public:
  ...
  constexpr InterfaceDescriptorParser(buf_view data)
